...
export async function createSale(input: CreateSaleInput): Promise<Sale> {
  ...
  await db.withTransactionAsync(async () => {
    await db.runAsync(
      `INSERT INTO sales (
        id, invoice_number, customer_id, subtotal, discount_amount, discount_percent,
    ...
    for (const item of input.items) {
      ...
      await db.runAsync(
        `INSERT INTO sale_items (
          id, sale_id, product_id, product_name, unit_price, cost_price,
          quantity, line_total, discount_amount
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [
          itemId,
          id,
