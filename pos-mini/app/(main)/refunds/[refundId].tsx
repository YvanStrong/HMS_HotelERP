  ...
  return (
    <ScreenContainer scroll>
        <View className="p-4" style={cardStyle}>
          <Text className="text-lg font-bold text-black">{refund.refundNumber}</Text>
          <Text className="text-sm text-gray-600">{new Date(refund.createdAt).toLocaleString()}</Text>
          {refund.saleInvoiceNumber ? (
            <Text className="mt-1 text-sm text-black">Sale: {refund.saleInvoiceNumber}</Text>
          ) : null}
          {refund.reason ? (
          ...
          <View className="flex-row justify-between">
            ...
            <Text className="text-lg font-bold text-black">{formatMoney(refund.total, settings)}</Text>
          </View>
        </View>
        {refund.saleId ? (
          <Pressable
            onPress={() => router.push(`/(main)/sales/${refund.saleId}`)}
            className="mt-4 border-2 border-black bg-white py-3"
          >
            <Text className="text-center font-bold text-black">View Original Sale</Text>
          </Pressable>
        ) : null}
        <Pressable
          onPress={() => router.push('/(main)/refunds/history')}
          className="mt-2 border-2 border-black bg-app-primary py-3"
        >
